...
def current_user(request: Request, db: Session):
    if settings.dev_bypass_auth:
        user = db.scalar(select(User).where(func.lower(User.username) == settings.dev_user.lower()))
        if not user:
...
async def home(request: Request, db: Session = Depends(get_db)):
    ...
    if not user:
        ...
        return templates.TemplateResponse('login.html', {'request': request, 'configured': configured(), 'plex_auth_url': plex_auth_url_value})
    if user.is_admin:
...
async def setup_get(request: Request, db: Session = Depends(get_db), reconnect: str = ''):
    ...
    if reconnecting or not values.get('plex_server_token'):
        ...
        if code:
            plex_auth_url_value = plex_auth_url(code, public_url('/auth/complete'))
    return templates.TemplateResponse('setup.html', {'request': request, 'values': values, 'user': current, 'services': service_context(values), 'plex_auth_url': plex_auth_url_value, 'reconnecting': reconnecting})
...
async def settings_test_service(request: Request, db: Session = Depends(get_db)):
    ...
    query = urlencode({'test_kind': base_kind, 'test_ok': '1' if ok else '0', 'test_message': message})
    return RedirectResponse(f'{return_to}{sep}{query}', status_code=303)
...
async def settings_import_tautulli(request: Request, db: Session = Depends(get_db)):
    ...
    try:
        ...
        ok = bool(result.message and not result.message.startswith('Tautulli URL') and not result.message.startswith('Tautulli API did not'))
        message = result.message
    except Exception as exc:
        ok = False
        message = f'Tautulli import failed: {exc.__class__.__name__}'
    sep = '&' if '?' in return_to else '?'
    query = urlencode({'import_ok': '1' if ok else '0', 'import_message': message})
    return RedirectResponse(f'{return_to}{sep}{query}', status_code=303)
...
async def trigger_download_search(request: Request, title: str = Form(...), request_type: str = Form(...), db: Session = Depends(get_db)):
    ...
    radarr, sonarr = await service_clients()
    try:
        if request_type == 'movie' and radarr:
            movie = await radarr.search_movie(title)
            if movie and movie.get('id'):
                await radarr.trigger_search(int(movie['id']))
        elif request_type == 'tv' and sonarr:
            series = await sonarr.search_series(title)
...
async def plex_image(path: str):
    ...
    async with httpx.AsyncClient(timeout=20) as client:
        ...
        return Response(content=resp.content, media_type=resp.headers.get('content-type', 'image/jpeg'), status_code=resp.status_code)
...
async def delete_library_item(
    ...
    try:
        ...
        pass
    target = return_to if return_to.startswith('/libraries') else '/libraries'
    return RedirectResponse(target, status_code=303)
