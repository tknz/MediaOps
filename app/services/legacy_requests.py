...
def import_legacy_requests(db: Session, seerr_path: str, radarr_path: str, sonarr_path: str) -> int:
    if not all([seerr_path, radarr_path, sonarr_path]):
        return 0
    s = sqlite3.connect(seerr_path); s.row_factory = sqlite3.Row
    r = sqlite3.connect(radarr_path); r.row_factory = sqlite3.Row
    so = sqlite3.connect(sonarr_path); so.row_factory = sqlite3.Row
    added = 0
    rows = s.execute('''
