...
AVAILABLE_STATUSES = {4, 5, '4', '5', 'available', 'partially_available', 'partially-available'}
...
def _request_status(item: dict) -> str:
    media_status = (item.get('media') or {}).get('status')
    if media_status in AVAILABLE_STATUSES:
        return 'available'
    return STATUS.get(item.get('status'), str(item.get('status', 'unknown')))
    ...
    async with httpx.AsyncClient(timeout=60) as client:
        ...
        while True:
            ...
            for item in items:
                ...
                if not row:
                    row = MediaRequest(source='seerr', source_id=sid, requester_name=requester_name, request_type=item.get('type','unknown'), title=title, status=status, requested_at=requested_at)
                    db.add(row)
                    changed += 1
